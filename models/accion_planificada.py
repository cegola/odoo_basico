...
class accion_planificada(models.Model):
    ...
    def listado_facturas(self):
        ...
        # usuario=__system que ten en res_partner name=OdooBot
        self.env.user.tz = self.env['res.partner'].search([('id', '=', 3)])[0].tz
        # como ten tz=UTC poñemoslle o tz do usuario=3 que é o administrador
        agora = self.env['odoo_basico_celia.informacion'].convirte_data_hora_de_utc_a_timezone_do_usuario(fields.Datetime.now())
        self.env.user.tz = 'UTC'  # deixamolo como estaba con tz=UTC

        # facturas_ids = self.search([('state', '=', 'open')]) Version Odoo 12
        facturas_ids = self.search([('state', '=', 'posted')]) #aqui ya estamos en el modelo de facturacion
        if facturas_ids:
            listado = ""
